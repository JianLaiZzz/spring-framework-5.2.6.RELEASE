 ...
 *
 * <p>
 * <strong>NOTE:</strong> As of 5.0 this class is in maintenance mode, with
...
public class RestTemplate extends InterceptingHttpAccessor implements RestOperations
{
	...
	private void validateConverters(List<HttpMessageConverter<?>> messageConverters)
	{
		...
		Assert.noNullElements(messageConverters,
			...
			throws RestClientException
	{
		...
		HttpMessageConverterExtractor<T> responseExtractor = new HttpMessageConverterExtractor<>(
			...
			throws RestClientException
	{
		...
		RequestCallback requestCallback = httpEntityCallback(requestEntity, responseType);
		ResponseExtractor<ResponseEntity<T>> responseExtractor = responseEntityExtractor(responseType);
		return nonNull(doExecute(requestEntity.getUrl(), requestEntity.getMethod(), requestCallback,
				responseExtractor));
	}
		...
		public void doWithRequest(ClientHttpRequest request) throws IOException
		{
			if (this.responseType != null)
			{
				List<MediaType> allSupportedMediaTypes = getMessageConverters().stream()
						...
						.flatMap(this::getSupportedMediaTypes).distinct()
	...
	private class HttpEntityRequestCallback extends AcceptHeaderRequestCallback
	{
		...
		public void doWithRequest(ClientHttpRequest httpRequest) throws IOException
		{
			...
			if (requestBody == null)
			{
				...
				{
					requestHeaders
							.forEach((key, values) -> httpHeaders.put(key, new LinkedList<>(values)));
				}
			...
			else
			{
				...
				for (HttpMessageConverter<?> messageConverter : getMessageConverters())
				{
					if (messageConverter instanceof GenericHttpMessageConverter)
					{
						...
						if (genericConverter.canWrite(requestBodyType, requestBodyClass,
								requestContentType))
						{
							if (!requestHeaders.isEmpty())
							{
							...
							genericConverter.write(requestBody, requestBodyType, requestContentType,
									httpRequest);
							return;
						}
					}
		...
		private void logBody(Object body, @Nullable MediaType mediaType,
