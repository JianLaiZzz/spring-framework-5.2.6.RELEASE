...
class HtmlCharacterEntityDecoder
{
	...
	{
		this.nextPotentialReferencePosition = Math.max(startPosition,
				this.nextSemicolonPosition - MAX_REFERENCE_SIZE);
		...
		do
		{
			...
			{
				this.nextSemicolonPosition = this.originalMessage.indexOf(';',
						this.nextPotentialReferencePosition + 1);
			}

			boolean isPotentialReference = (this.nextPotentialReferencePosition != -1
					&& this.nextSemicolonPosition != -1 && this.nextPotentialReferencePosition
							- this.nextSemicolonPosition < MAX_REFERENCE_SIZE);
	...
	private void processPossibleReference()
	{
		if (this.nextPotentialReferencePosition != -1)
		{
			...
			boolean wasProcessable = isNumberedReference ? processNumberedReference()
