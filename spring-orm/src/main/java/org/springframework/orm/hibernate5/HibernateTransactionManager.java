		...
		implements ResourceTransactionManager, BeanFactoryAware, InitializingBean
{
	 ...
	 * <p>
	 * <b>NOTE: For scenarios with many transactions that just read data from
	...
	protected void doBegin(Object transaction, TransactionDefinition definition)
	{
		...
		try
		{
			...
			{
				TransactionSynchronizationManager.bindResource(obtainSessionFactory(),
		...
		catch (Throwable ex)
		{
			...
			throw new CannotCreateTransactionException("Could not open Hibernate Session for transaction",
					ex);
		}
	}
	...
	protected boolean isPhysicallyConnected(Session session)
	{
		...
		return ((SessionImplementor) session).getJdbcCoordinator().getLogicalConnection()
	...
	private static final class SuspendedResourcesHolder
	{
		...
		private SuspendedResourcesHolder(SessionHolder sessionHolder,
				@Nullable ConnectionHolder conHolder)
		{
			this.sessionHolder = sessionHolder;
