...
public class HibernateJpaDialect extends DefaultJpaDialect {
	...
	static {
		try {
			...
			try {
				...
				throw new IllegalStateException("No compatible Hibernate getFlushMode signature found",
						ex2);
			}
		}
	 ...
	 * <p>
	 * <b>NOTE:</b> The default behavior in terms of read-only handling changed
	...
	public Object beginTransaction(EntityManager entityManager, TransactionDefinition definition)
		...
		boolean isolationLevelNeeded = (definition
		...
		if (isolationLevelNeeded || definition.isReadOnly()) {
			if (this.prepareConnection) {
				...
				previousIsolationLevel = DataSourceUtils.prepareConnectionForTransaction(preparedCon,
						definition);
	...
	protected DataAccessException convertHibernateAccessException(HibernateException ex) {
		...
		if (ex instanceof LockAcquisitionException) {
			...
			return new CannotAcquireLockException(ex.getMessage() + "; SQL [" + jdbcEx.getSQL() + "]",
					ex);
		}
		...
		if (ex instanceof StaleObjectStateException) {
			...
			return new ObjectOptimisticLockingFailureException(hibEx.getEntityName(),
					hibEx.getIdentifier(), ex);
		}
	...
	private static class HibernateConnectionHandle implements ConnectionHandle {
		...
		public static Connection doGetConnection(Session session) {
			try {
				...
				throw new IllegalStateException("Cannot find connection() method on Hibernate Session",
						ex);
			}
		}
	}
