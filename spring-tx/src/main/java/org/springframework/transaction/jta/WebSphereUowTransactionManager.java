		...
		implements CallbackPreferringPlatformTransactionManager
{
	...
	protected UOWManager lookupUowManager(String uowManagerName) throws TransactionSystemException
	{
		...
		catch (NamingException ex)
		{
			throw new TransactionSystemException(
					"WebSphere UOWManager is not available at JNDI location [" + uowManagerName + "]",
					ex);
		}
	}
	...
	protected UOWManager lookupDefaultUowManager() throws TransactionSystemException
	{
		try
		{
			logger.debug("Retrieving WebSphere UOWManager from default JNDI location ["
			...
			List<TransactionSynchronization> synchronizations)
	{
		...
		obtainUOWManager().registerInterposedSynchronization(
			...
			throws TransactionException
	{
		...
		try
		{
			...
			action = new UOWActionAdapter<>(def, callback,
		...
		catch (UOWException | UOWActionException ex)
		{
			TransactionSystemException tse = new TransactionSystemException(
					"UOWManager transaction processing failed", ex);
	...
	private class UOWActionAdapter<T> implements UOWAction, SmartTransactionObject
	{
		...
		public UOWActionAdapter(TransactionDefinition definition, TransactionCallback<T> callback,
				boolean actualTransaction, boolean newTransaction, boolean newSynchronization,
				boolean debug)
		{
		...
		public void run()
		{
			...
			DefaultTransactionStatus status = prepareTransactionStatus(this.definition,
					(this.actualTransaction ? this : null), this.newTransaction, this.newSynchronization,
					this.debug, null);
			try
			{
