	...
	public void addTransactionalMethod(Class<?> clazz, String mappedName, TransactionAttribute attr)
	{
		...
		for (Method method : matchingMethods)
		{
					...
					|| (!regMethodName.equals(name) && regMethodName.length() <= name.length()))
			{
				...
				{
					logger.debug("Replacing attribute for transactional method [" + method
							+ "]: current name '" + name + "' is more specific than '" + regMethodName
							+ "'");
				}
			...
			else
			{
				...
				{
					logger.debug(
							"Keeping attribute for transactional method [" + method + "]: current name '"
