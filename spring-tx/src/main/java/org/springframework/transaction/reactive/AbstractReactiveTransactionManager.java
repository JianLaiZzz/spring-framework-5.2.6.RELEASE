		...
		implements ReactiveTransactionManager, Serializable
{
		...
		return TransactionSynchronizationManager.forCurrentTransaction().flatMap(synchronizationManager ->
		{
					...
					|| def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED)
			{
				...
				return TransactionContextManager.currentContext()
						...
						.flatMap(nestedSynchronizationManager -> suspend(nestedSynchronizationManager,
								null).map(Optional::of).defaultIfEmpty(Optional.empty())
										.flatMap(suspendedResources ->
										{
											...
											return Mono.defer(() ->
											{
												GenericReactiveTransaction status = newReactiveTransaction(
														nestedSynchronizationManager, def, transaction,
														true, debugEnabled,
														suspendedResources.orElse(null));
												return doBegin(nestedSynchronizationManager, transaction,
														def).doOnSuccess(
																ignore -> prepareSynchronization(
			...
			else
			{
				...
				return Mono.just(prepareReactiveTransaction(synchronizationManager, def, null, true,
						debugEnabled, null));
			}
		});
	}
			...
			Object transaction, boolean debugEnabled) throws TransactionException
	{
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NOT_SUPPORTED)
		{
			...
			return suspend
					.map(suspendedResources -> prepareReactiveTransaction(synchronizationManager,
							definition, null, false, debugEnabled, suspendedResources)) //
					.switchIfEmpty(
							Mono.fromSupplier(() -> prepareReactiveTransaction(synchronizationManager,
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW)
		{
			...
			return suspendedResources.flatMap(suspendedResourcesHolder ->
			{
				...
				return doBegin(synchronizationManager, transaction, definition).doOnSuccess(
		...
		return Mono.just(prepareReactiveTransaction(synchronizationManager, definition, transaction,
	...
	private GenericReactiveTransaction prepareReactiveTransaction(
			TransactionSynchronizationManager synchronizationManager, TransactionDefinition definition,
			@Nullable Object transaction, boolean newTransaction, boolean debug,
			@Nullable Object suspendedResources)
	{

		GenericReactiveTransaction status = newReactiveTransaction(synchronizationManager, definition,
				transaction, newTransaction, debug, suspendedResources);
			...
			@Nullable Object suspendedResources)
	{
		...
		return new GenericReactiveTransaction(transaction, newTransaction,
				!synchronizationManager.isSynchronizationActive(), definition.isReadOnly(), debug,
				suspendedResources);
	}
			...
			throws TransactionException
	{
		...
		if (resourcesHolder != null)
		{
			...
			if (suspendedSynchronizations != null)
			{
				...
				return resume
						.then(doResumeSynchronization(synchronizationManager, suspendedSynchronizations));
			}
		}
			...
			TransactionSynchronizationManager synchronizationManager)
	{
		...
		List<TransactionSynchronization> suspendedSynchronizations = synchronizationManager
				.getSynchronizations();
		...
		Mono<Object> commit = prepareForCommit(synchronizationManager, status)
				...
				})).then(Mono.empty().onErrorResume(ex ->
				{
					...
					else if (ErrorPredicates.RUNTIME_OR_ERROR.test(ex))
					{
						...
						result = mono
								.then(doRollbackOnCommitException(synchronizationManager, status, ex))
			...
			Throwable ex) throws TransactionException
	{
		...
		return Mono.defer(() ->
		{
			...
			else if (status.hasTransaction())
			{
				if (status.isDebug())
				{
					logger.debug("Marking existing transaction as rollback-only after commit exception",
							ex);
				}
		...
		}).onErrorResume(ErrorPredicates.RUNTIME_OR_ERROR, rbex ->
		{
			...
			return triggerAfterCompletion(synchronizationManager, status,
					TransactionSynchronization.STATUS_UNKNOWN).then(Mono.error(rbex));
		}).then(triggerAfterCompletion(synchronizationManager, status,
				TransactionSynchronization.STATUS_ROLLED_BACK));
	}
			...
			GenericReactiveTransaction status)
	{
		...
		if (status.isNewSynchronization())
		{
			...
			return TransactionSynchronizationUtils
					.invokeAfterCommit(synchronizationManager.getSynchronizations());
		}
			...
			GenericReactiveTransaction status, int completionStatus)
	{
		...
		if (status.isNewSynchronization())
		{
			List<TransactionSynchronization> synchronizations = synchronizationManager
					.getSynchronizations();
			synchronizationManager.clearSynchronization();
			...
			Object transaction) throws TransactionException
	{
		...
		throw new TransactionSuspensionNotSupportedException("Transaction manager ["
				+ getClass().getName() + "] does not support transaction suspension");
	}
	...
	protected static final class SuspendedResourcesHolder
	{
		...
		private SuspendedResourcesHolder(@Nullable Object suspendedResources,
				List<TransactionSynchronization> suspendedSynchronizations, @Nullable String name,
				boolean readOnly, @Nullable Integer isolationLevel, boolean wasActive)
		{
