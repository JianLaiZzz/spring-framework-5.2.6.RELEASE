...
abstract class AbstractListenerContainerParser implements BeanDefinitionParser
{
	...
	public BeanDefinition parse(Element element, ParserContext parserContext)
	{
		...
		MutablePropertyValues specificProperties = parseSpecificContainerProperties(element,
				parserContext);
		...
		if (StringUtils.hasText(factoryId))
		{
			...
			if (beanDefinition != null)
			{
				...
				parserContext
						.registerBeanComponent(new BeanComponentDefinition(beanDefinition, factoryId));
			}
		}
		...
		for (int i = 0; i < childNodes.getLength(); i++)
		{
			...
			if (child.getNodeType() == Node.ELEMENT_NODE)
			{
				...
				{
					parseListener(element, (Element) child, parserContext, commonProperties,
							specificProperties);
				}
			}
		}
			...
			MutablePropertyValues commonContainerProperties, PropertyValues specificContainerProperties)
	{
		...
		{
			parserContext.getReaderContext().error("Listener 'ref' attribute contains empty value.",
					listenerEle);
		}
		else
		{
			...
			ParserContext parserContext)
	{
		...
		{
			parserContext.getReaderContext()
					.error("Invalid listener container 'response-destination-type': only "
	...
	protected Integer parseAcknowledgeMode(Element ele, ParserContext parserContext)
	{
		...
		if (StringUtils.hasText(acknowledge))
		{
			...
			{
				parserContext.getReaderContext()
						.error("Invalid listener container 'acknowledge' setting [" + acknowledge
