...
public class SimpleJmsHeaderMapper extends AbstractHeaderMapper<Message> implements JmsHeaderMapper {
	...
	private static final Set<Class<?>> SUPPORTED_PROPERTY_TYPES = new HashSet<>(
			Arrays.asList(Boolean.class, Byte.class, Double.class, Float.class, Integer.class, Long.class,
	...
	public void fromHeaders(MessageHeaders headers, javax.jms.Message jmsMessage) {
		try {
			...
			Destination jmsReplyTo = getHeaderIfAvailable(headers, JmsHeaders.REPLY_TO,
					Destination.class);
			if (jmsReplyTo != null) {
				try {
					...
					if (value != null && SUPPORTED_PROPERTY_TYPES.contains(value.getClass())) {
						try {
							...
							if (headerName.startsWith("JMSX")) {
								...
								logger.debug("Failed to map message header '" + headerName
										+ "' to JMS property", ex);
							}
						}
					}
