...
public abstract class ConnectionFactoryUtils {
	...
	public static void releaseConnection(@Nullable Connection con, @Nullable ConnectionFactory cf,
		...
		if (started && cf instanceof SmartConnectionFactory
	 ...
	 */
	public static boolean isSessionTransactional(@Nullable Session session,
												 @Nullable ConnectionFactory cf) {
	 ...
	 * Obtain a JMS Session that is synchronized with the current transaction, if any.
	 * <p>
	 * This {@code doGetTransactionalSession} variant always starts the underlying
	...
	public static Session doGetTransactionalSession(ConnectionFactory connectionFactory,
		...
		if (resourceHolderToUse != resourceHolder) {
			TransactionSynchronizationManager
					.registerSynchronization(new JmsResourceSynchronization(resourceHolderToUse,
	...
	private static class JmsResourceSynchronization
		...
		protected void processResourceAfterCommit(JmsResourceHolder resourceHolder) {
			try {
				...
				throw new SynchedLocalTransactionFailedException("Local JMS transaction failed to commit",
						ex);
			}
		}
