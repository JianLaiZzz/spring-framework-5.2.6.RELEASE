 ...
 *
 * <p>
 * <b>NOTE: The {@code ConnectionFactory} used with this template should
...
public class JmsTemplate extends JmsDestinationAccessor implements JmsOperations {
	...
	public Destination getDefaultDestination() {
		return (this.defaultDestination instanceof Destination ? (Destination) this.defaultDestination
				: null);
	}
	 ...
	 * <p>
	 * Specify {@link #RECEIVE_TIMEOUT_NO_WAIT} (or any other negative value)
	 ...
	 * <p>
	 * The default is -1 (no delivery delay passed on to the broker).
	 ...
	 * <p>
	 * Use {@code execute(SessionCallback)} for the general case.
	...
	protected Message doReceive(Session session, Destination destination,
								@Nullable String messageSelector) throws JMSException {
	...
	protected Message doReceive(Session session, MessageConsumer consumer) throws JMSException {
		try {
			...
			if (connectionFactory != null) {
				resourceHolder = (JmsResourceHolder) TransactionSynchronizationManager
	 ...
	 */
	@Nullable
	protected Message doSendAndReceive(Session session, Destination destination,
	...
	public <T> T browseSelected(final String queueName, @Nullable final String messageSelector,
		...
		return execute(session ->
		{
			Queue queue = (Queue) getDestinationResolver().resolveDestinationName(session, queueName,
					false);
			QueueBrowser browser = createBrowser(session, queue, messageSelector);
			try {
				return action.doInJms(session, browser);
