...
abstract class AbstractJmsAnnotationDrivenTests
{
	...
	protected void testFullConfiguration(ApplicationContext context)
	{
		JmsListenerContainerTestFactory simpleFactory = context.getBean("simpleFactory",
				JmsListenerContainerTestFactory.class);
		assertThat(simpleFactory.getListenerContainers().size()).isEqualTo(1);
		MethodJmsListenerEndpoint endpoint = (MethodJmsListenerEndpoint) simpleFactory
				.getListenerContainers().get(0).getEndpoint();
	...
	protected void testCustomConfiguration(ApplicationContext context)
	{
		...
		assertThat(endpoint.getClass()).as("Wrong endpoint type")
		...
		assertThat(customRegistry.getListenerContainerIds().size())
				.as("Wrong number of containers in the registry").isEqualTo(2);
	...
	protected void testJmsListenerRepeatable(ApplicationContext context)
	{
		...
		MethodJmsListenerEndpoint first = (MethodJmsListenerEndpoint) simpleFactory
				.getListenerContainer("first").getEndpoint();
		...
		MethodJmsListenerEndpoint second = (MethodJmsListenerEndpoint) simpleFactory
				.getListenerContainer("second").getEndpoint();
