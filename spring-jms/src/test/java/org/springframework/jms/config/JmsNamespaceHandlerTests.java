...
public class JmsNamespaceHandlerTests
{
	...
	public void testBeansCreated()
	{
		...
		Map<String, JmsListenerContainerFactory> containerFactories = context
				.getBeansOfType(JmsListenerContainerFactory.class);
		assertThat(containerFactories.size())
				.as("Context should contain 3 JmsListenerContainerFactory instances").isEqualTo(3);
	}
	...
	public void testContainerConfiguration() throws Exception
	{
		...
		assertThat(defaultConnectionFactoryCount)
				.as("1 container should have the default connectionFactory").isEqualTo(1);
		assertThat(explicitConnectionFactoryCount)
				.as("2 containers should have the explicit connectionFactory").isEqualTo(2);
	}

	@Test
	public void testJcaContainerConfiguration() throws Exception
	{
		Map<String, JmsMessageEndpointManager> containers = context
				.getBeansOfType(JmsMessageEndpointManager.class);
		...
		Object messageListener = new DirectFieldAccessor(endpointFactory)
				.getPropertyValue("messageListener");
		assertThat(messageListener.getClass()).as("Wrong message listener")
	...
	public void testJmsContainerFactoryConfiguration()
	{
		...
		DefaultMessageListenerContainer container = factory
				.createListenerContainer(createDummyEndpoint());
	...
	public void testJcaContainerFactoryConfiguration()
	{
		...
		assertThat(container.getActivationSpecConfig().getMessageConverter())
				...
				.isEqualTo(context.getBean("testMessageConverter"));
		assertThat(container.isPubSubDomain()).as("Wrong pub/sub").isEqualTo(true);
		assertThat(container.getActivationSpecConfig().getMaxConcurrency()).as("wrong concurrency")
				.isEqualTo(5);
		assertThat(container.getActivationSpecConfig().getPrefetchSize()).as("Wrong prefetch")
				.isEqualTo(50);
	...
	public void testRecoveryInterval()
	{
		...
		long recoveryInterval3 = getRecoveryInterval(
	...
	{
		// JMS
		DefaultMessageListenerContainer listener0 = this.context.getBean(
				DefaultMessageListenerContainer.class.getName() + "#0",
		...
		assertThat(listener0.getConcurrentConsumers())
				.as("Wrong concurrency on listener using placeholder").isEqualTo(2);
		assertThat(listener0.getMaxConcurrentConsumers())
				.as("Wrong concurrency on listener using placeholder").isEqualTo(3);
		assertThat(listener1.getConcurrentConsumers()).as("Wrong concurrency on listener1").isEqualTo(3);
		assertThat(listener1.getMaxConcurrentConsumers()).as("Wrong max concurrency on listener1")
				.isEqualTo(5);
		assertThat(listener2.getConcurrentConsumers()).as("Wrong custom concurrency on listener2")
				.isEqualTo(5);
		assertThat(listener2.getMaxConcurrentConsumers()).as("Wrong custom max concurrency on listener2")
				.isEqualTo(10);

		// JCA
		JmsMessageEndpointManager listener3 = this.context.getBean("listener3",
				JmsMessageEndpointManager.class);
		JmsMessageEndpointManager listener4 = this.context.getBean("listener4",
				JmsMessageEndpointManager.class);
		assertThat(listener3.getActivationSpecConfig().getMaxConcurrency())
				.as("Wrong concurrency on listener3").isEqualTo(5);
		assertThat(listener4.getActivationSpecConfig().getMaxConcurrency())
				.as("Wrong custom concurrency on listener4").isEqualTo(7);
	}
	...
	public void testResponseDestination()
	{
		...
		assertThat(listener1.isReplyPubSubDomain()).as("Wrong response destination type on listener1")
				.isEqualTo(false);
		assertThat(listener2.isReplyPubSubDomain()).as("Wrong response destination type on listener2")
				.isEqualTo(false);
		...
		assertThat(listener3.isReplyPubSubDomain()).as("Wrong response destination type on listener3")
				.isEqualTo(false);
		assertThat(listener4.isReplyPubSubDomain()).as("Wrong response destination type on listener4")
				.isEqualTo(false);
	}
	...
	public void testErrorHandlers()
	{
		...
		ErrorHandler defaultErrorHandler = getErrorHandler(
				DefaultMessageListenerContainer.class.getName() + "#0");
	...
	public void testComponentRegistration()
	{
		...
		assertThat(context
				.containsComponentDefinition(DefaultMessageListenerContainer.class.getName() + "#0"))
	...
	public void testSourceExtraction()
	{
		...
		while (iterator.hasNext())
		{
			...
			assertThat(compDef.getSource()).as(
					"CompositeComponentDefinition '" + compDef.getName() + "' has no source attachment")
					.isNotNull();
	...
	private static class ToolingTestApplicationContext extends ClassPathXmlApplicationContext
	{
		...
		protected void initBeanDefinitionReader(XmlBeanDefinitionReader beanDefinitionReader)
		{
			...
			beanDefinitionReader
					.setEventListener(new StoringReaderEventListener(this.registeredComponents));
